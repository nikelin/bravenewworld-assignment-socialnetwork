  ...
  override def requestFriendsList(accessToken: Option[AccessToken], userId: UserAccountId)(implicit ec: ExecutionContext): Future[Iterable[PersonWithAttributes]] = {
    userId match {
      case userId: UserAccountId.FacebookId =>
        ...
        seleniumDriversPool.tryAcquire(1.minute) match {
          case Some(driverLease) ⇒
            driverLease { driver ⇒
              ...
              "scrolling" timing {
                ...
                do {
                  jsExecutor.executeScript("window.scrollTo(0, document.body.offsetHeight + 200)")

                  elementPresent = try {
                    Await.result(Future(new WebDriverWait(driver, 20.seconds.toMillis)
                      .until(ExpectedConditions.presenceOfElementLocated(By.className("seeMoreFriends")))), 5.seconds)
                    true
                  } catch {
